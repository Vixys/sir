...
fn file_doesnt_exist() -> Result<(), Box<dyn std::error::Error>> {
    ...
    cmd.assert()
        .failure()
        .stderr(predicate::str::contains("rcat: test/file/doesnt/exist: No such file or directory (os error 2)\n"));
...
fn display_content_in_multiple_files_with_one_missing() -> Result<(), Box<dyn std::error::Error>> {
    ...
    cmd.arg(file.as_os_str()).arg("__does_not_exit__.txt").arg(file2.as_os_str());

    cmd.assert()
        .failure()
        .stderr(predicate::str::contains("rcat: __does_not_exit__.txt: No such file or directory (os error 2)\n"))
        .stdout(predicate::str::contains("file_content\nfile2_content\n"));
